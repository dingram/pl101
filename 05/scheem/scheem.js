...
initialEnv = (function() {
	...
	add_func_binding(initEnv, '=number?', 2, 2, function(args, env) {
		var o1 = evalScheem(args[0], env);
		var o2 = evalScheem(args[1], env);
		return (
			(typeof o1 == 'number') &&
			(typeof o2 == 'number') &&
			(o1 == o2)
	...
	// type checks
	add_func_binding(initEnv, 'eq?',      2, 2, function(args, env) { return (evalScheem(args[0], env) == evalScheem(args[1], env)) ? '#t' : '#f'; });
	...
	// list functions
	add_func_binding(initEnv, 'list',   1, undefined, function(args, env) {
		return args.map(function(a){ return evalScheem(a, env); });
	});
	...
	add_func_binding(
		...
		function(args, env) {
			var val = evalScheem(args[0], env);
			(typeof alert != 'undefined' ? alert : console.error)(val.join(' '));
			return val;
		}
	);
